...
// Define types for unit conversion and tracked data
type UnitConversion = {
  Symbol: string;
  Company: string;
  LastPrice: string;
...
export default function DataTable() {
    ...
    const fetchData = async () => {
      ...
      for (const company of data) {
        convertedData.push({
          ...
          Change: `${company.currency} ${sign(company.stock.change)}${
    ...
    columnHelper.accessor("PercentChg", {
      cell: (info) => (
        <Badge
          fontSize="0.9em"
          bg={
            parseFloat(info.getValue().replace("%", "")) >= 0
              ? "green.200"
              : "red.200"
          }
        >
  ...
  return <DataGrid data={trackedData} columns={columns}></DataGrid>;
}
