"""
This module includes functions to ingest news articles on a regular basis,
saving the newly ingested articles to the database,
and sending article emails for important articles once the job has finished.
"""
import logging
...
def saveAnalysedArticles(articles: List[AnalysisService.AnalysedArticle]):
    ...
    with conn.cursor() as cur:
        for article in articles:
            ...
            cur.execute(
                """SELECT CompanyID FROM company WHERE TickerCode=%s""",
            ...
            cur.execute(
                """SELECT SourceID FROM web_source WHERE SourceURL=%s""", [domain]
            )
            result = cur.fetchone()
            if result is None:
                # source hasn't been inserted as site doesn't have a popularity - may have been an error in the process
                cur.execute(
                    """INSERT INTO web_source (SourceName, SourceURL) VALUES (%s, %s) RETURNING SourceID""",
            ...
            keywordsAsText = (
                str([x["name"] for x in article.keywords])
                .replace("[", "")
                .replace("]", "")
            )
            ...
            cur.execute(
                """INSERT INTO company_articles (CompanyID, ArticleID) VALUES (%s, %s)""",
                [company_id, articleID],
            )
            ...
            update_company_score = """UPDATE company 
                                    ...
                                    """
            cur.execute(
                update_company_score, (company_id, date_30_days_ago, company_id)
            )
            conn.commit()
            ...
            if abs(article.score) >= SCORE_THRESHOLD:
                ...
                cur.execute(
                    """INSERT INTO notifications (ArticleID) VALUES (%s)""",
                    (articleID,),
                )
...
def job():
    ...
    and sending emails for important articles
    """
    ...
    if newartleIDs:
        data = {
            "recipients": ["stockapp220@gmail.com"],
            "articleList": newartleIDs,
        }  # CHANGE

        response = requests.post(
            "http://localhost:5000/sendarticleemail",
