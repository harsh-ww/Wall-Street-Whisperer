"""
This module contains all flask routes that get company information.
"""
from flask import Blueprint, request, jsonify
from services.AlphaVantageService import (
    getCompanyDetails,
    companySearch,
    getCompanyDetailsNonUS,
...
def get_company_details_db(ticker: str):
    """
    Return the name and current company score from the company table, 
    which only contains tracked companies.
    """
    conn = get_db_connection()
    with conn.cursor() as cur:
        cur.execute(
            "SELECT CompanyName, CurrentScore FROM company WHERE TickerCode = %s",
            [ticker],
        )
        result = cur.fetchone()
...
def company_details(ticker: str):
    ...
    Each company is made unique by its ticker
    """
...
def search_companies():
    """
    Return all company matches when given a query string.
    The companies are got from the database and alpha vantage api
    """
...
def search_companies_db(ticker):
    """
    Return companies that either the name matches the search query, 
    or the ticker code matches the query
    """
...
def get_timeSeries(ticker):
    """
    Return the past data of a certain company
    To be used in displaying the graph
    """
    ...
    if not data:
        return (
            jsonify(
                {
                    "message": "Ticker code does not exist, or invalid granularity provided."
                }
            ),
            404,
        )
...
def changeFormat(data):
    """
    Reformat the timeseries into a list of dictionary
    And return the most recent 30 data and in correct order
    """
    formatted_data = []
