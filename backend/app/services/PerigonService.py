"""
This module contains functions that uses the perigon API to fetch news articles.
"""
import requests
...
def getCompanyNewsPerigon(
    companyName: str, timePeriodHours: int, count: int, topSources: bool = False
) -> list[Article]:
    ...
    if response.status_code != 200:
        logging.error(
            f"Failed Perigon news fetching. Error {response.status_code} - {response.json()}"
        )
