"""
This module contains definitions and algorithms for calculating the 
score of a company, by analysing the sentiment results of the article
and the popularity of the websource.
"""
import os
import logging
...
class BatchArticleAnalysis:
    ...
    def splitArticleTarget(
        self, companyName: str, articleText: str
    ) -> tuple[str, str, str]:
        """
    ...
    def fetchPopularity(self, domain: str):
        ...
        if response.status_code == 404:
            ...
            if response.status_code != 200:
                logging.error(
                    f"Failed Similarweb fetching. Error {response.status_code} - {response.text} "
                )
                return None
    ...
    def updatePopularity(self, domain: str, popularity: int, name: str = None):
        ...
        with conn.cursor() as cur:
            ...
            if name is not None:
                ...
                cur.execute(
                    insertQuery, [name, domain, popularity, datetime.now().isoformat()]
                )
            else:
                # existing entries already have a name
                cur.execute(
                    updateQuery, [popularity, datetime.now().isoformat(), domain]
                )
            # save changes
            conn.commit()
