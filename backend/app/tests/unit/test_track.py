...
def test_save_tracked(clear_data):
    result = track.save_tracked_company(
        "IBM", "International Business Machines", "IBM", "NYSE"
    )
...
class TestGetCompanyInfo:
    @mock.patch(
        "services.AlphaVantageService.getCompanyDetailsNonUS", side_effect=mockedAVNonUS
    )
    @mock.patch(
        "services.AlphaVantageService.getCompanyDetails", side_effect=mockedAVUS
    )
